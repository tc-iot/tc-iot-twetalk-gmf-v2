...
int IOT_DataTemplate_Init(void *client, IotDataTemplateCallback callback, void *usr_data)
{
    ...
    DataTemplateContext data_template_context;
    data_template_context.usr_data = usr_data;

    data_template_context.property_callback = callback.property_callback;
    rc = data_template_topic_check_and_sub(client, DATA_TEMPLATE_TYPE_PROPERTY, data_template_context);

    if (callback.event_callback.method_event_reply_callback) {
        data_template_context.event_callback = callback.event_callback;
        rc |= data_template_topic_check_and_sub(client, DATA_TEMPLATE_TYPE_EVENT, data_template_context);
    }

    if (callback.action_callback.method_action_callback) {
        data_template_context.action_callback = callback.action_callback;
        rc |= data_template_topic_check_and_sub(client, DATA_TEMPLATE_TYPE_ACTION, data_template_context);
    }
    return rc;
}
...
int IOT_DataTemplate_Register(void *client, IotDataTemplateCallback callback, void *usr_data)
{
    ...
    data_template_context.property_callback = callback.property_callback;
    rc = data_template_context_register(client, DATA_TEMPLATE_TYPE_PROPERTY, data_template_context);

    if (callback.event_callback.method_event_reply_callback) {
        data_template_context.event_callback = callback.event_callback;
        rc |= data_template_context_register(client, DATA_TEMPLATE_TYPE_EVENT, data_template_context);
    }

    if (callback.action_callback.method_action_callback) {
        data_template_context.action_callback = callback.action_callback;
        rc |= data_template_context_register(client, DATA_TEMPLATE_TYPE_ACTION, data_template_context);
    }
    return rc;
}
...
int IOT_DataTemplate_PropertyGetModeDefine(void *client, char *buf, int buf_len)
{
    ...
    PropertyPublishParams publish_params = {0};
    return data_template_property_publish(client, PROPERTY_UP_METHOD_TYPE_GET_MODE_DEFINE, buf, buf_len,
                                          publish_params);
}
