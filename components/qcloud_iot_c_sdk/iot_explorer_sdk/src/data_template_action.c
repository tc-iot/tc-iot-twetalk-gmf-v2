...
typedef struct {
    ...
    UtilsJsonValue action_id;
    UtilsJsonValue params;
} ActionParams;

static UtilsListResult _action_callback(void *list, void *val, void *usr_data)
{
    DataTemplateContext *context       = (DataTemplateContext *)val;
    ActionParams        *action_params = (ActionParams *)usr_data;
    context->action_callback.method_action_callback(action_params->client_token, action_params->action_id,
                                                    action_params->params, context->usr_data);
...
void data_template_action_message_handler(void *client, const MQTTMessage *message, void *usr_data)
{
    ...
    ActionParams action_params = {0};
    Log_d("receive action message:%.*s", message->payload_len, message->payload_str);

    rc = utils_json_check_value("method", sizeof("method") - 1, "action", sizeof("action") - 1, message->payload_str,
                                message->payload_len);
    if (rc) {
        goto error;
    }
    ...
    utils_json_value_get("params", strlen("params"), message->payload_str, message->payload_len, &action_params.params);

    utils_list_process(usr_data, LIST_HEAD, _action_callback, &action_params);
    return;
error:
