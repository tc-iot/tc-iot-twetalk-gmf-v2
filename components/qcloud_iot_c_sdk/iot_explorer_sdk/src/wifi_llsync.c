...
static int _set_wifi_info(const char *ssid, size_t ssid_len, const char *pwd, size_t pwd_len, void *usr_data)
{
    ...
    LlsyncWifiConfigResult *config_result   = (LlsyncWifiConfigResult *)usr_data;
    config_result->bind_time->get_ssid_time = TCI_HAL_GetTimeMs();
    memcpy(config_result->info->ssid, ssid, ssid_len);
    memcpy(config_result->info->pwd, ssid, ssid_len);
    int rc = TCI_HAL_Wifi_StaInfoSet(ssid, ssid_len, pwd, pwd_len);
    if (rc) {
        config_result->result      = rc;
        config_result->recv_result = TCIOT_BOOL_TRUE;
    }
...
static int _connect_wifi(size_t timeout_ms, void *usr_data)
{
    ...
    LlsyncWifiConfigResult *config_result         = (LlsyncWifiConfigResult *)usr_data;
    config_result->bind_time->wifi_connected_time = TCI_HAL_GetTimeMs();

    int rc = TCI_HAL_Wifi_StaConnect(timeout_ms);
    if (rc) {
        config_result->result      = rc;
        config_result->recv_result = TCIOT_BOOL_TRUE;
    }
...
static int _set_wifi_token(const char *token, size_t token_len, void *usr_data)
{
    ...
    LlsyncWifiConfigResult *config_result    = (LlsyncWifiConfigResult *)usr_data;
    config_result->bind_time->get_token_time = TCI_HAL_GetTimeMs();
                           ...
                           uint64_t timeout_ms)
{
    ...
    if (!iot_llsync_is_init()) {
        TCIOTBLELLsyncInitParams llsync_init_params = DEFAULT_LLSYNC_INIT_PARAMS;
        static DeviceInfo      dev_info;
        TCI_HAL_GetDevInfo((uint8_t *)&dev_info, sizeof(DeviceInfo));
    ...
    LlsyncWifiConfigResult config_result = {
        .info        = info,
        .bind_time   = bind_time,
        .recv_result = TCIOT_BOOL_FALSE,
        .result      = QCLOUD_ERR_FAILURE,
    };

    TCIOTBLELLsyncWifiCallback callback = {
    ...
    do {
        TCI_HAL_SleepMs(100);
    } while (!TCI_HAL_TimerExpired(&timer) && !config_result.recv_result);
