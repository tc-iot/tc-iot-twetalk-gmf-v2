...
static int _utils_hmac_sha1_process(const uint8_t *msg, int msg_len, const uint8_t *key, int key_len, uint8_t *out)
{
    IotSha1Context context;
    uint8_t        k_ipad[KEY_IO_PAD_SIZE]; /* inner padding - key XORd with ipad */
    uint8_t        k_opad[KEY_IO_PAD_SIZE]; /* outer padding - key XORd with opad */
    ...
    utils_sha1_update(&context, msg, msg_len);            /* then text of datagram */
 ...
 */
int utils_hmac_sha1(const uint8_t *msg, int msg_len, const uint8_t *key, int key_len, uint8_t *digest)
{
 ...
 * @return 0 for success
 */
int utils_hmac_sha1_hex(const uint8_t *msg, int msg_len, const uint8_t *key, int key_len, char *digest)
{
    unsigned char out[SHA1_DIGEST_SIZE] = {0};
    if (!msg || !digest || !key) {
        return -1;
 ...
 * @param[in] key key using in hmac-sha1
 * @param[in] key_len key len
 * @param[out] output output to calculate
 * @return 0 for success
 */
int utils_hmac_sha256(const uint8_t *msg, int msg_len, const uint8_t *key, int key_len, uint8_t output[32])
{
    IotSha256Context context;
    uint8_t          k_ipad[KEY_IO_PAD_SIZE]; /* inner padding - key XORd with ipad  */
    uint8_t          k_opad[KEY_IO_PAD_SIZE]; /* outer padding - key XORd with opad */
    int32_t          i;
    ...
    utils_sha256_update(&context, k_ipad, KEY_IO_PAD_SIZE); /* start with inner pad */
    ...
    utils_sha256_update(&context, k_opad, KEY_IO_PAD_SIZE);      /* start with outer pad */
    utils_sha256_update(&context, output, SHA256_DIGEST_LENGTH); /* then results of 1st hash */
