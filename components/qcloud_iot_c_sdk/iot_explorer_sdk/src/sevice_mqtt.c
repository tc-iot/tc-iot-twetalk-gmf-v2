...
static int _service_type_match(void *val, void *usr_data)
{
    ServiceRegisterParams *params = val;
    ServiceType            type   = *(ServiceType *)usr_data;
    return params->type == type;
}

static int _service_control_match(void *val, void *usr_data)
{
    ServiceRegisterParams *params  = val;
    ServiceListControl    *control = usr_data;
    return params->type == control->type;
}
 ...
 */
static UtilsListResult _server_list_unregister_callback(void *list, void *val, void *usr_data)
{
 ...
 * @return @see UtilsListResult
 */
static UtilsListResult _server_list_process_message_callback(void *list, void *val, void *usr_data)
{
    ServiceRegisterParams *params  = (ServiceRegisterParams *)val;
 ...
 * @return @see UtilsListResult
 */
static UtilsListResult _server_list_get_usr_data(void *list, void *val, void *usr_data)
{
    ServiceRegisterParams *params  = val;
    ServiceListControl    *control = usr_data;
    control->usr_data              = params->usr_data;
    return LIST_TRAVERSE_BREAK;
}
 ...
 * @return @see IotReturnCode
 */
int service_mqtt_service_register(void *client, ServiceRegisterParams *params)
{
    ServiceMqttContext *context = _service_mqtt_context_get(client);
    if (!context) {
        return QCLOUD_ERR_FAILURE;
    }
    return utils_list_push_copy(context->service_list, params, sizeof(ServiceRegisterParams)) ? QCLOUD_ERR_MALLOC
                                                                                              : QCLOUD_RET_SUCCESS;
}
...
void service_mqtt_service_unregister(void *client, ServiceType type)
{
    ...
    utils_list_process_match(context->service_list, LIST_HEAD, _server_list_unregister_callback, &type,
                             _service_type_match);
}

/**
 * @brief Get usr data registered.
 *
 * @param[in,out] client pointer to mqtt client
 * @param[in] type @see ServiceType
 * @return NULL or usr_data in ServiceRegisterParams
 */
void *service_mqtt_service_get_usr_data(void *client, ServiceType type)
{
    ServiceMqttContext *context = _service_mqtt_context_get(client);
    if (!context) {
        return NULL;
    }
    ServiceListControl control = {0};
    control.type               = type;
    utils_list_process_match(context->service_list, LIST_HEAD, _server_list_get_usr_data, &control,
                             _service_control_match);
    return control.usr_data;
}
