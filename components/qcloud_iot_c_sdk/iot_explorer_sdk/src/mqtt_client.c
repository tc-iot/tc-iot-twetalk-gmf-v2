...
static int _mqtt_client_list_init(QcloudIotClient *client)
{
    ...
    client->list_sub_wait_ack = utils_list_create(func, QCLOUD_IOT_MQTT_MAX_MESSAGE_HANDLERS);
...
{
    if (params->host) {
        HAL_Snprintf(client->host_addr, HOST_STR_LENGTH, "%s", params->host);
    } else {
        // default host
...
static int _qcloud_iot_mqtt_client_init(QcloudIotClient *client, const MQTTInitParams *params)
{
    ...
    rc = HAL_Snprintf(client->client_id, MAX_SIZE_OF_CLIENT_ID, "%s%s", client->device_info->product_id,
...
IotBool IOT_MQTT_IsSubReady(void *client, const char *topic_filter)
{
    ...
    STRING_PTR_SANITY_CHECK(topic_filter, IOT_BOOL_FALSE);
