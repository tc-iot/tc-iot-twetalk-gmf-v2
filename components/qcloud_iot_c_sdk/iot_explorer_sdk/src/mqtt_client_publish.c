...
static int _construct_pub_info(void *val, void *usr_data, size_t size)
{
    QcloudIotPubInfo *repub_info    = val;
    PushPubInfo      *push_pub_info = usr_data;
 ...
 *
 * @param[in,out] push_pub_info @see PushPubInfo
...
int qcloud_iot_mqtt_publish(QcloudIotClient *client, const char *topic_name, const PublishParams *params)
{
    ...
    if (params->qos > QOS0) {
        PushPubInfo push_pub_info = {.client = client, .packet_len = packet_len, .packet_id = packet_id, .val = &val};
