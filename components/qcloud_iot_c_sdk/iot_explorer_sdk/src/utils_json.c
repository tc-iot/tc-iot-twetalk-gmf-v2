 ...
 * @param[in,out] remain_len remaining length
 * @param[in] delimiter begin delimiter @see JsonDelimiter
 * @param[out] value value node
 * @return 0 for success
 */
static int _get_json_value(char *key_end, int *remain_len, JsonDelimiter delimiter, json_value *value)
{
    char *value_begin = _find_json_delimiter(delimiter, JSON_DELIMITER_SPACE, key_end, remain_len);
    if (!value_begin) {
        return -1;
    }
 ...
 * @return 0 for success
 */
static int _json_get_value_by_key(const char *str, int str_len, const char *key, int key_len, json_value *value)
{
    int      rc           = 0;
    ...
    do {
        ...
        rc = _get_json_value(key_found.pos_end, &str_len, JSON_DELIMITER_VALUE, value);
        if (rc) {
            break;
        }
...
int utils_json_value_get(const char *key, int key_len, const char *src, int src_len, UtilsJsonValue *value)
{
    ...
    while ((delim = strchr(key_iter, '.')) != NULL) {
        ...
        rc = _json_get_value_by_key(src_iter, src_iter_len, key_next, key_next_len, &value_tmp);
        if (rc) {
            return rc;
        }
    ...
    rc = _json_get_value_by_key(src_iter, src_iter_len, key_iter, key_len + key - key_iter, &value_tmp);
    if (rc) {
        return rc;
    }
...
int utils_json_value_data_get(UtilsJsonValue value, UtilsJsonValueType type, void *data)
{
    ...
    switch (type) {
        case UTILS_JSON_VALUE_TYPE_UINT8:
            return !(sscanf(value_tmp, "%" SCNu8, (uint8_t *)data) == 1);
        case UTILS_JSON_VALUE_TYPE_INT:
            return !(sscanf(value_tmp, "%d", (int *)data) == 1);
 ...
 * @return 0 for success
 */
int utils_json_get_int(const char *key, int key_len, const char *src, int src_len, int *data)
{
    int            rc;
    UtilsJsonValue tmp;
    ...
    rc = utils_json_value_data_get(tmp, UTILS_JSON_VALUE_TYPE_INT, data);
    if (rc) {
        return -1;
    }
    return 0;
 ...
 */
void utils_json_array_parse(const char *src, int src_len, UtilsJsonArrayIterResult (*obj_cb)(UtilsJsonValue, void *),
                            void *arg)
{
#define SKIP_SPACE(str) while (str != NULL && *str == ' ' && (str = str + 1))
    ...
    json_value    value     = {JSON_ELEMENT_ARRAY, NULL, NULL, 0};
    JsonDelimiter delimiter = JSON_DELIMITER_ARRAY_BEGIN;
    while (start && start <= (src + src_len)) {
        remain_length = src + src_len - start;
        if (_get_json_value(start, &remain_length, delimiter, &value)) {
            break;
        }
        UtilsJsonValue value_json = {.value = value.pos_begin, .value_len = value.element_len};
        if (UTILS_JSON_ARRAY_ITER_STOP == obj_cb(value_json, arg)) {
            return;
        }

        start     = value.pos_end;
        delimiter = JSON_DELIMITER_ELEMENT_END;
    }
}

/**
 * @brief Check if value matched.
 *
 * @param[in] key key in json, support nesting with '.'
 * @param[in] key_len key len
 * @param[in] dst_value dst value to compare
 * @param[in] dst_value_len dst value len
 * @param[in] src json string
 * @param[in] src_len src length
 * @return 0 for success
 */
int utils_json_check_value(const char *key, int key_len, const void *dst_value, int dst_value_len, const char *src,
                           int src_len)
{
    UtilsJsonValue value;
    if (utils_json_value_get(key, key_len, src, src_len, &value)) {
        return -1;
    }
    return value.value_len == dst_value_len && !memcmp(value.value, dst_value, dst_value_len) ? 0 : -1;
}
