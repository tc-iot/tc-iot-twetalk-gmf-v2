...
/**
 * @brief Push context to list.
 *
 * @param[in] list context list
 * @param[in] context context to push
 * @return @see IotReturnCode
 */
static int _data_template_context_push(void *list, DataTemplateContext context)
{
    return utils_list_push_copy(list, &context, sizeof(DataTemplateContext)) ? QCLOUD_ERR_MALLOC : QCLOUD_RET_SUCCESS;
}
 ...
 * @return @see IotReturnCode
 */
int data_template_topic_check_and_sub(void *client, DataTemplateType type, DataTemplateContext context)
{
    OnMessageHandler msg_handle[] = {
        [DATA_TEMPLATE_TYPE_PROPERTY] = data_template_property_message_handler,
        [DATA_TEMPLATE_TYPE_EVENT]    = data_template_event_message_handler,
        [DATA_TEMPLATE_TYPE_ACTION]   = data_template_action_message_handler,
    };
    ...
    if (_data_template_context_push(data_template_context_list, context)) {
        utils_list_destroy(data_template_context_list);
        return QCLOUD_ERR_MALLOC;
    }

    SubscribeParams sub_params    = DEFAULT_SUB_PARAMS;
    sub_params.on_message_handler = msg_handle[type];
    sub_params.qos                = QOS1;
...
/**
 * @brief Register context to context list.
 *
 * @param[in,out] client pointer to mqtt client
 * @param[in] type @see DataTemplateType
 * @param[in] context @see DataTemplateContext
 * @return @see IotReturnCode
 */
int data_template_context_register(void *client, DataTemplateType type, DataTemplateContext context)
{
    // register topic
    char data_template_topic[MAX_SIZE_OF_CLOUD_TOPIC];
    _data_template_topic_generate(data_template_topic, MAX_SIZE_OF_CLOUD_TOPIC, DATA_TEMPLATE_DIRECTION_DOWN, type,
