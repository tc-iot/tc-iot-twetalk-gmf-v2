...
#ifndef IOT_HUB_DEVICE_C_SDK_INCLUDE_COMPONENT_QCLOUD_TCIOT_LLSYNC_CONFIG_H_
#define IOT_HUB_DEVICE_C_SDK_INCLUDE_COMPONENT_QCLOUD_TCIOT_LLSYNC_CONFIG_H_
...
// choose the ability you used, set 1 with BLE_QTCIOT_LLSYNC_STANDARD and BLE_QTCIOT_LLSYNC_CONFIG_NET will enable dual-mode
// communication
#define BLE_QTCIOT_LLSYNC_STANDARD   0  // support llsync standard
#define BLE_QTCIOT_LLSYNC_CONFIG_NET 1  // support llsync configure network
#define BLE_QTCIOT_LLSYNC_GATEWAY    0  // support llsync gateway

#if (1 == BLE_QTCIOT_LLSYNC_STANDARD) && (1 == BLE_QTCIOT_LLSYNC_CONFIG_NET)
#define BLE_QTCIOT_LLSYNC_DUAL_COM 1  // support llsync dual communication, do not use ble ota under the mode
#endif

#define BLE_QTCIOT_SDK_VERSION "1.5.1"  // sdk version
#define BLE_QTCIOT_SDK_DEBUG   0        // sdk debug
...
// stopped automatically in a few minutes if the device is not bind, define BLE_QTCIOT_BUTTON_BROADCAST is 1 and
// BLE_QTCIOT_BIND_TIMEOUT is the period that broadcast stopped.
// if the device in the bound state, broadcast dose not stop automatically.
#define BLE_QTCIOT_BUTTON_BROADCAST 0
#if BLE_QTCIOT_BUTTON_BROADCAST
#define BLE_QTCIOT_BIND_TIMEOUT (2 * 60 * 1000)  // unit: ms
#endif                                         // BLE_QTCIOT_BUTTON_BROADCAST

// in some BLE stack the default ATT_MTU is 23, set BLE_QTCIOT_REMOTE_SET_MTU is 1 if you want to reset the mtu by the
// Tencent Lianlian. Tencent Lianlian will set the mtu get from function ble_get_user_data_mtu_size()
#define BLE_QTCIOT_REMOTE_SET_MTU (1)
...
#define BLE_QTCIOT_EVENT_MAX_SIZE (128)
// the minimum between BLE_QTCIOT_EVENT_MAX_SIZE and mtu
#define BLE_QTCIOT_EVENT_BUF_SIZE (23)
...
// use property
#define BLE_QTCIOT_INCLUDE_PROPERTY

#define BLE_QTCIOT_DYNREG_ENABLE 0
// some users hope to confirm on the device before the binding, set BLE_QTCIOT_SECURE_BIND is 1 to enable the secure
...
#define BLE_QTCIOT_SECURE_BIND 0
#if BLE_QTCIOT_SECURE_BIND
#define BLE_QTCIOT_BIND_WAIT_TIME 60
...
#define BLE_QTCIOT_USER_DEVELOPER_VERSION "0.0.1"

#define BLE_LLSYNC_CORE_DATA_FILEPATH "/data/ble_llsync_core_data.txt"

#define BLE_QTCIOT_SUPPORT_OTA 0  // 1 is support ota, others not
...
#undef BLE_QTCIOT_RECORD_FLASH_ADDR
// some sdk info needs to stored on the device and the address is up to you
#define BLE_QTCIOT_RECORD_FLASH_ADDR \
    (TX_PPSP_IMPL_CFGS_PROG_ADDR_BASE + TX_PPSP_IMPL_CFGS_PROG_ADDR_BGNS - TX_PPSP_IMPL_CFGS_PROG_SCTR_SIZE)  // 0xFE000

#define BLE_QTCIOT_RECORD_FLASH_PAGESIZE 4096  // flash page size, see chip datasheet

#if BLE_QTCIOT_SUPPORT_OTA
#define BLE_QTCIOT_SUPPORT_RESUMING 0  // 1 is support resuming, others not
#if BLE_QTCIOT_SUPPORT_RESUMING
// storage ota info in the flash if support resuming ota file
#define BLE_QTCIOT_OTA_INFO_FLASH_ADDR (BLE_QTCIOT_RECORD_FLASH_ADDR + 0x1000)
#endif  // BLE_QTCIOT_SUPPORT_RESUMING

#define BLE_QTCIOT_TOTAL_PACKAGES 32  // the total package numbers in a loop
#define BLE_QTCIOT_PACKAGE_LENGTH 128  // the user data length in package, ble_get_user_data_mtu_size() - 3 is the max
#define BLE_QTCIOT_RETRY_TIMEOUT  0x5   // the max interval between two packages, unit: second
// the time spent for device reboot, the server waiting the device version reported after upgrade. unit: second
#define BLE_QTCIOT_REBOOT_TIME      20
#define BLE_QTCIOT_PACKAGE_INTERVAL 20  // the interval between two packages send by the server
// the package from the server will storage in the buffer, write the buffer to the flash at one time when the buffer
// overflow. reduce the flash write can speed up file download, we suggest the BLE_QTCIOT_OTA_BUF_SIZE is multiples
// of BLE_QTCIOT_PACKAGE_LENGTH and equal flash page size
#define BLE_QTCIOT_OTA_BUF_SIZE (BLE_QTCIOT_RECORD_FLASH_PAGESIZE)
#endif  // BLE_QTCIOT_SUPPORT_OTA
#endif  // BLE_QTCIOT_LLSYNC_STANDARD

#if BLE_QTCIOT_LLSYNC_DUAL_COM
#define TCIOT_BLE_UUID_SERVICE 0xFFE8
#else
#if BLE_QTCIOT_LLSYNC_STANDARD
#define TCIOT_BLE_UUID_SERVICE 0xFFE0
#endif  // BLE_QTCIOT_LLSYNC_STANDARD
#if BLE_QTCIOT_LLSYNC_CONFIG_NET
#define TCIOT_BLE_UUID_SERVICE 0xFFF0
...
#endif  // IOT_HUB_DEVICE_C_SDK_INCLUDE_COMPONENT_QCLOUD_TCIOT_LLSYNC_CONFIG_H_
