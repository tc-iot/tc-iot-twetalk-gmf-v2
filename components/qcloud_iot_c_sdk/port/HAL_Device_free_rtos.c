...
static IotBool sg_is_set_device_info = IOT_BOOL_FALSE;
...
int HAL_SetDevInfo(DeviceInfo *device_info)
{
    ...
    if (rc) {
        ...
        sg_is_set_device_info = IOT_BOOL_TRUE;
        // save to file.
        rc = HAL_File_Write(DEFAULT_DEVICE_INFO_FILEPATH, device_info, sizeof(DeviceInfo), 0);
        if (!rc) {
            Log_e("save device info err");
            rc = QCLOUD_ERR_DEV_INFO;
        } else {
...
int HAL_GetDevInfo(DeviceInfo *device_info)
{
    ...
    if (sg_is_set_device_info) {
        ...
        rc |= device_info_copy(device_info->device_secret, sg_device_secret,
                               MAX_SIZE_OF_DEVICE_SECRET);  // get dev secret
#ifdef DEV_DYN_REG_ENABLED
        rc |= device_info_copy(device_info->product_secret, sg_product_secret,
        ...
        // read from file
        rc = HAL_File_Read(DEFAULT_DEVICE_INFO_FILEPATH, device_info, sizeof(DeviceInfo), 0);
        if (!rc) {
            Log_e("read %s fail.", DEFAULT_DEVICE_INFO_FILEPATH);
            rc = QCLOUD_ERR_DEV_INFO;
        } else {
            rc = QCLOUD_RET_SUCCESS;
        }
