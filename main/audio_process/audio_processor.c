...
#if defined CONFIG_ENCODER_FORMAT_G711A || defined CONFIG_DECODER_FORMAT_G711U
#include "esp_g711_enc.h"
#elif defined CONFIG_ENCODER_FORMAT_OPUS
#include "esp_opus_enc.h"
#elif defined CONFIG_ENCODER_FORMAT_AAC
#include "esp_aac_enc.h"
#endif /* CONFIG_ENCODER_FORMAT_G711A || CONFIG_DECODER_FORMAT_G711A */
#if defined CONFIG_DECODER_FORMAT_OPUS
#include "esp_opus_dec.h"
#elif defined CONFIG_DECODER_FORMAT_AAC
#include "esp_aac_dec.h"
#elif defined CONFIG_DECODER_FORMAT_G711A || defined CONFIG_DECODER_FORMAT_G711U
#include "esp_g711_dec.h"
#endif  /* CONFIG_ENCODER_FORMAT_OPUS */
...
#else
#define RECORD_CHANNELS            CONFIG_ENCODER_CHANNEL_COUNT
#define RECORD_SAMPLE_BITS         CONFIG_ENCODER_BIT_WIDTH
#define RECORD_SAMPLE_RATE         CONFIG_ENCODER_SAMPLE_RATE
#endif
...
#if defined CONFIG_ENCODER_FORMAT_G711A || defined CONFIG_ENCODER_FORMAT_G711U \
                                       || defined CONFIG_ENCODER_FORMAT_PCM
#define AUDIO_RECORD_PIP_TASK_STACK_SIZE    (5120)
#else
#define AUDIO_RECORD_PIP_TASK_STACK_SIZE    (40 * 1024)
#endif  /* CONFIG_ENCODER_FORMAT_G711A || CONFIG_ENCODER_FORMAT_G711U */
#define AUDIO_RECORD_PIP_TASK_PRIORITY      (5)
#define AUDIO_RECORD_PIP_TASK_CORE          (0)

#if defined CONFIG_DECODER_FORMAT_G711A || defined CONFIG_DECODER_FORMAT_G711U
#define AUDIO_PLAYBACK_PIP_TASK_STACK_SIZE  (5120)
#else
#define AUDIO_PLAYBACK_PIP_TASK_STACK_SIZE  (40 * 1024)
#endif  /* CONFIG_ENCODER_FORMAT_G711A || CONFIG_ENCODER_FORMAT_G711U */
#define AUDIO_PLAYBACK_PIP_TASK_PRIORITY    (19)
...
typedef struct {
    ...
    esp_gmf_db_handle_t bus2;
    audio_mixer_v2_handle_t mixer;
} audio_mixer_t;
...
static void aec_debug_data_write(char *data, int len)
{
    if (record_flag) {
        if (file_socket == -1) {
            file_socket = open(AEC_DEBUDG_FILE_NAME, O_WRONLY | O_CREAT | O_TRUNC, 0666);
            if (file_socket == -1) {
                ESP_LOGI(TAG, "Cannot open file, reason: %s\n", strerror(errno));
                return;
            }
        }
        write(file_socket, data, len);
...
esp_err_t audio_manager_init(void)
{
    ...
    audio_manager.sdcard_handle = NULL;
    ...
    ret = esp_codec_dev_set_in_channel_gain(audio_manager.rec_dev, BOARD_CODEC_REF_MIC, DEFAULT_RECOPRD_REF_DB);
    ESP_GMF_RET_ON_NOT_OK(TAG, ret, { ESP_LOGW(TAG, "Failed to set record reference gain, continuing..."); }, "Failed to set record reference gain");
...
#if 1 //CONFIG_ENABLE_RECORDER_DEBUG
    // Teardown SD card
...
static int prompt_out_data_callback(uint8_t *data, int data_size, void *ctx)
{
    if (audio_mixer.mixer) {
        ...
        esp_gmf_db_acquire_write(audio_mixer.bus, &_blk, data_size, portMAX_DELAY);
        _blk.buf = (uint8_t *)data;
        _blk.valid_size = data_size;
        esp_gmf_db_release_write(audio_mixer.bus, &_blk, portMAX_DELAY);
    } else {
...
{
#ifndef CONFIG_KEY_PRESS_DIALOG_MODE
    srmodel_list_t *models = esp_srmodel_init("model"); // "model"    /sdcard/model/wake_word
    ...
    // audio_recorder.afe_cfg->ns_init = true;
    // audio_recorder.afe_cfg->ns_model_name = "WebRTC";
    // audio_recorder.afe_cfg->afe_ns_mode = AFE_NS_MODE_WEBRTC;

    audio_recorder.afe_cfg->aec_init = true;
    audio_recorder.afe_cfg->vad_init = VAD_ENABLE;
    audio_recorder.afe_cfg->vad_mode = VAD_MODE_4;
...
{
#if CONFIG_KEY_PRESS_DIALOG_MODE
    #if defined CONFIG_ENCODER_FORMAT_PCM
        static const char *pcm_elements[] = {"aud_bit_cvt", "aud_rate_cvt", "aud_ch_cvt"};
        *elements = pcm_elements;
        *element_count = sizeof(pcm_elements) / sizeof(char *);
        *output_name = "aud_ch_cvt";
        *input_name = "aud_bit_cvt";
    #else
        static const char *encoded_elements[] = {"aud_bit_cvt", "aud_rate_cvt", "aud_ch_cvt", "aud_enc"};
        *elements = encoded_elements;
        *element_count = sizeof(encoded_elements) / sizeof(char *);
        *output_name = "aud_enc";
        *input_name = "aud_bit_cvt";
    #endif
#else
        *input_name = "ai_afe";
    #if defined CONFIG_ENCODER_FORMAT_G711A || defined CONFIG_ENCODER_FORMAT_G711U
        static const char *g711_elements[] = {"ai_afe", "aud_rate_cvt", "aud_enc"};
        *elements = g711_elements;
        *element_count = sizeof(g711_elements) / sizeof(char *);
        *output_name = "aud_enc";
    #elif defined CONFIG_ENCODER_FORMAT_OPUS
        static const char *opus_elements[] = {"ai_afe", "aud_enc"};
        *elements = opus_elements;
        *element_count = sizeof(opus_elements) / sizeof(char *);
        *output_name = "aud_enc";
    #elif defined CONFIG_ENCODER_FORMAT_AAC
        static const char *aac_elements[] = {"ai_afe", "aud_enc"};
        *elements = aac_elements;
        *element_count = sizeof(aac_elements) / sizeof(char *);
        *output_name = "aud_enc";
    #else
...
static esp_err_t recorder_configure_g711_encoder(void)
{
    ...
    esp_gmf_info_sound_t info = {
        ...
        .channels = 1,
        .bits = 16,
    #if defined CONFIG_ENCODER_FORMAT_G711A
        .format_id = ESP_AUDIO_TYPE_G711A,
    #elif defined CONFIG_ENCODER_FORMAT_G711U
...
{
#if defined CONFIG_ENCODER_FORMAT_OPUS
    esp_gmf_element_handle_t enc_handle = NULL;
    esp_gmf_pipeline_get_el_by_name(audio_recorder.pipe, "aud_enc", &enc_handle);

    esp_opus_enc_config_t opus_enc_cfg = ESP_OPUS_ENC_CONFIG_DEFAULT();
    opus_enc_cfg.bitrate = CONFIG_ENCODER_OPUS_BITRATE;
    opus_enc_cfg.channel = CONFIG_ENCODER_CHANNEL_COUNT;
    opus_enc_cfg.sample_rate = CONFIG_ENCODER_SAMPLE_RATE;
    opus_enc_cfg.frame_duration = ESP_OPUS_ENC_FRAME_DURATION_60_MS;
#if CONFIG_ENCODER_OPUS_ENABLE_VBR
    opus_enc_cfg.enable_vbr = true;
#else
    opus_enc_cfg.enable_vbr = false;
#endif  // CONFIG_ENCODER_OPUS_ENABLE_VBR
    ...
    esp_gmf_pipeline_get_el_by_name(audio_recorder.pipe, "aud_rate_cvt", &rate_cvt);
    if (rate_cvt) {
        esp_gmf_rate_cvt_set_dest_rate(rate_cvt, CONFIG_ENCODER_SAMPLE_RATE);
    }
...
static esp_err_t recorder_configure_aac_encoder(void)
{
#if defined CONFIG_ENCODER_FORMAT_AAC
    esp_gmf_element_handle_t enc_handle = NULL;
    esp_gmf_pipeline_get_el_by_name(audio_recorder.pipe, "aud_enc", &enc_handle);

    esp_aac_enc_config_t aac_enc_cfg = ESP_AAC_ENC_CONFIG_DEFAULT();
    aac_enc_cfg.bitrate = CONFIG_ENCODER_AAC_BITRATE;
    aac_enc_cfg.channel = CONFIG_ENCODER_CHANNEL_COUNT;
    aac_enc_cfg.sample_rate = CONFIG_ENCODER_SAMPLE_RATE;
#if CONFIG_ENCODER_AAC_ENABLE_VBR
    // aac_enc_cfg.enable_vbr = true;
#else
    // aac_enc_cfg.enable_vbr = false;
#endif  // CONFIG_ENCODER_AAC_ENABLE_VBR

    esp_audio_enc_config_t aac_cfg = {
        .type = ESP_AUDIO_TYPE_AAC,
        .cfg_sz = sizeof(esp_aac_enc_config_t),
        .cfg = &aac_enc_cfg,
    };
    esp_gmf_audio_enc_reconfig(enc_handle, &aac_cfg);
...
esp_err_t audio_recorder_open(recorder_event_callback_t cb, void *ctx)
{
    ...
    err = recorder_configure_aac_encoder();
    ESP_GMF_RET_ON_NOT_OK(TAG, err, { goto cleanup_and_exit; }, "Failed to configure AAC encoder");
    ...
    audio_recorder.state = AUDIO_PLAYER_STATE_PLAYING;
...
esp_err_t audio_playback_feed_data(uint8_t *data, int data_size)
{
    ...
    esp_gmf_data_bus_block_t blk = {0};
#if CONFIG_DECODER_FORMAT_PCM || CONFIG_DECODER_FORMAT_AAC
    ...
    ret = esp_gmf_db_release_write(audio_playback.fifo, &blk, portMAX_DELAY);
    ESP_GMF_RET_ON_NOT_OK(TAG, ret, { return ESP_FAIL; }, "Failed to release write to playback FIFO");
#endif  /* CONFIG_DECODER_FORMAT_PCM */
    return ESP_OK;
}
...
static int playback_outport_release_write(void *handle, esp_gmf_data_bus_block_t *blk, int block_ticks)
{
    if (audio_mixer.mixer) {
        esp_gmf_data_bus_block_t _blk = {0};
        esp_gmf_db_acquire_write(audio_mixer.bus2, &_blk, blk->valid_size, block_ticks);
        _blk.buf = (uint8_t *)blk->buf;
        _blk.valid_size = blk->valid_size;
        esp_gmf_db_release_write(audio_mixer.bus2, &_blk, block_ticks);
...
{
#ifdef CONFIG_DECODER_FORMAT_OPUS
    ...
    opus_dec_cfg.frame_duration = CONFIG_DECODER_OPUS_FRAME_DURATION_MS;
    opus_dec_cfg.channel = CONFIG_DECODER_CHANNEL_COUNT;
    opus_dec_cfg.sample_rate = CONFIG_DECODER_SAMPLE_RATE;
    ...
    esp_gmf_info_sound_t info = {
        .sample_rates = CONFIG_DECODER_SAMPLE_RATE,
        .channels = CONFIG_DECODER_CHANNEL_COUNT,
        .bits = CONFIG_DECODER_BIT_WIDTH,
...
static esp_err_t playback_configure_g711_decoder(void)
{
    ...
    esp_gmf_info_sound_t info = {
        ...
        .channels = 1,
        .bits = 16,
    #if defined CONFIG_DECODER_FORMAT_G711A
        .format_id = ESP_AUDIO_SIMPLE_DEC_TYPE_G711A,
    #elif defined CONFIG_DECODER_FORMAT_G711U
...
static esp_err_t playback_configure_aac_decoder(void)
{
#if defined CONFIG_DECODER_FORMAT_AAC
    esp_gmf_element_handle_t dec_el = NULL;
    esp_gmf_pipeline_get_el_by_name(audio_playback.pipe, "aud_dec", &dec_el);
    if (!dec_el) {
        ESP_LOGE(TAG, "Failed to get AAC decoder element");
        return ESP_FAIL;
    }
    // Configure AAC decoder
    esp_aac_dec_cfg_t aac_dec_cfg = { 0 };
    aac_dec_cfg.sample_rate = CONFIG_DECODER_SAMPLE_RATE;
    aac_dec_cfg.channel = CONFIG_DECODER_CHANNEL_COUNT;
    aac_dec_cfg.bits_per_sample = CONFIG_DECODER_BIT_WIDTH;
#if CONFIG_DECODER_AAC_DISABLE_ADTS_HEADER
    aac_dec_cfg.no_adts_header = true;
#else
    aac_dec_cfg.no_adts_header = false;
#endif  // CONFIG_DECODER_AAC_ENABLE_ADTS
#if CONFIG_DECODER_AAC_ENABLE_PLUS
    aac_dec_cfg.aac_plus_enable = true;
#else
    aac_dec_cfg.aac_plus_enable = false;
#endif  // CONFIG_DECODER_AAC_ENABLE_PLUS
    
    esp_audio_simple_dec_cfg_t dec_cfg = {
        .dec_type = ESP_AUDIO_TYPE_AAC,
        .dec_cfg = &aac_dec_cfg,
    ...
    esp_err_t ret = esp_gmf_audio_dec_reconfig(dec_el, &dec_cfg);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Failed to configure AAC decoder");
        return ESP_FAIL;
    }
    ...
    ESP_GMF_RET_ON_NOT_OK(TAG, ret, { return ESP_FAIL; }, "Failed to configure AAC decoder");
...
{
#if defined CONFIG_DECODER_FORMAT_PCM
    esp_gmf_element_handle_t dec_el = NULL;
    esp_gmf_pipeline_get_el_by_name(audio_playback.pipe, "aud_dec", &dec_el);
    if (!dec_el) {
...
static esp_err_t playback_configure_converters(void)
{
    ...
    if (bit_cvt_el) {
        if (audio_mixer.mixer) {
            esp_gmf_bit_cvt_set_dest_bits(bit_cvt_el, 16);
        } else {
    ...
    if (ch_cvt_el) {
        if (audio_mixer.mixer) {
            esp_gmf_ch_cvt_set_dest_channel(ch_cvt_el, 1);
        } else {
...
esp_err_t audio_playback_open(void)
{
...
#if CONFIG_DECODER_FORMAT_PCM ||  CONFIG_DECODER_FORMAT_AAC
    esp_err_t err = esp_gmf_db_new_ringbuf(1, 1536, &audio_playback.fifo);
#else
    ...
    ESP_GMF_RET_ON_NOT_OK(TAG, err, { goto cleanup_and_exit; }, "Failed to configure OPUS decoder");

    err = playback_configure_g711_decoder();
    ESP_GMF_RET_ON_NOT_OK(TAG, err, { goto cleanup_and_exit; }, "Failed to configure G711 decoder");

    err = playback_configure_aac_decoder();
    ESP_GMF_RET_ON_NOT_OK(TAG, err, { goto cleanup_and_exit; }, "Failed to configure AAC decoder");
...
static void audio_mixer_callback_v2(void *arg, uint8_t *samples, int sample_bytes)
{
    if (audio_manager.play_dev) {
        if (sample_bytes > 0) {
            esp_codec_dev_write(audio_manager.play_dev, samples, sample_bytes);
        }
    }
}
...
{
    audio_mixer_v2_cfg_t cfg = DEFAULT_AUDIO_MIXER_V2_CONFIG();
    cfg.out_cb = audio_mixer_callback_v2;
    cfg.ctx = NULL;
    cfg.nb_streams = 2;
    cfg.bus = (esp_gmf_db_handle_t *)malloc(sizeof(esp_gmf_db_handle_t) * 2); // TODO: Need to free this

    esp_gmf_db_new_ringbuf(1, 4096 << 1, &audio_mixer.bus);
    assert(audio_mixer.bus);
    esp_gmf_db_new_ringbuf(1, 4096 << 1, &audio_mixer.bus2);
    assert(audio_mixer.bus2);
    cfg.bus[0] = audio_mixer.bus;
    cfg.bus[1] = audio_mixer.bus2;
    audio_mixer_v2_new(&cfg, &audio_mixer.mixer);
    audio_mixer_v2_start(audio_mixer.mixer);
    return ESP_OK;
}

esp_err_t audio_mixer_close(void)
{
    audio_mixer_v2_stop(audio_mixer.mixer);
    audio_mixer_v2_destroy(audio_mixer.mixer);
    esp_gmf_db_deinit(audio_mixer.bus);
    esp_gmf_db_deinit(audio_mixer.bus2);
    free(audio_mixer.bus);
    return ESP_OK;
}

esp_err_t audio_mixer_volume_adjust(audio_mixer_volume_adjust_t adjust)
{
    if (audio_mixer.mixer) {
        if (adjust == AUDIO_MIXER_FG_INCREASE) {
            audio_mixer_v2_set_volume_adjust(audio_mixer.mixer, audio_mixer.bus, AUDIO_VOLUME_INCREASE);
            audio_mixer_v2_set_volume_adjust(audio_mixer.mixer, audio_mixer.bus2, AUDIO_VOLUME_DECREASE);
        } else if (adjust == AUDIO_MIXER_BG_INCREASE) {
            audio_mixer_v2_set_volume_adjust(audio_mixer.mixer, audio_mixer.bus, AUDIO_VOLUME_DECREASE);
            audio_mixer_v2_set_volume_adjust(audio_mixer.mixer, audio_mixer.bus2, AUDIO_VOLUME_INCREASE);
        } else if (adjust == AUDIO_MIXER_NORMAL) {
            audio_mixer_v2_set_volume_adjust(audio_mixer.mixer, audio_mixer.bus, AUDIO_VOLUME_NORMAL);
            audio_mixer_v2_set_volume_adjust(audio_mixer.mixer, audio_mixer.bus2, AUDIO_VOLUME_NORMAL);
        }
    }
    return ESP_OK;
}
...
void audio_set_volume(int volume)
{
    if (audio_manager.play_dev) {
        ...
        esp_codec_dev_set_out_vol(audio_manager.play_dev, volume);
    }
}